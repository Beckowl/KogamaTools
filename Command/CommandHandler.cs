    ...
    internal static bool TryExecuteCommand(string commandLine)
    {
        ...
        switch (errorCode)
        {
            ...
            case CommandResult.InvalidArgs:
                NotificationHelper.WarnUser($"[{string.Join(", ", components.Skip(1))}] is not a valid combination of arguments for {command.Names.First()}. Type \"{command.Names.First()} ?\" for detailed info.");
                return true;
        }
